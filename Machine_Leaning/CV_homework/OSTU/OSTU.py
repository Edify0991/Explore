# 参考https://blog.csdn.net/sinat_29018995/article/details/114175427
import numpy as np
import cv2 as cv

# 将图片转为灰度图
# https://blog.csdn.net/weixin_43635647/article/details/99625105
img = cv.imread('scenary.jpeg', 0)
cv.imshow("img", img)


def OTSU(img_gray, GrayScale):
    assert img_gray.ndim == 2, "must input a gary_img"  # shape有几个数字, ndim就是多少，assert用来判断紧跟着的代码的正确性
    img_gray = np.array(img_gray).ravel().astype(np.uint8)  # ravel将图像拉成一维数组
    u1 = 0.0  # 背景像素的平均灰度值
    u2 = 0.0  # 前景像素的平均灰度值
    th = 0.0
