# 参考https://zhuanlan.zhihu.com/p/97689424
import cv2
import numpy as np
    ...
    for row in range(rows_[0], rows_[1], rows_[2]):
        for col in range(cols_[0], cols_[1], cols_[2]):
            ...
            for offset in offsets:
                ...
                if neighbor_val < 0.5:
                    continue
                label = neighbor_val if neighbor_val < label else label  # 寻找邻域内最小的label
            if label == 255:
                label_idx += 1
                label = label_idx
...
def recursive_seed(binary_img: np.array, seed_row, seed_col, offsets, num, max_num=100):
    ...
    for offset in offsets:
        ...
        if var < max_num:
            continue
        binary_img = recursive_seed(binary_img, neighbor_row, neighbor_col, offsets, num, max_num)  # 递归实现栈操作
    return binary_img
...
def Seed_Filling(binary_img, neighbor_hoods, max_num=100):
    ...
    for row in range(rows):
        for col in range(cols):
            var = binary_img[row][col]
            if var <= max_num:  # 由于是顺序遍历，所以如果var <= max_num，说明该点为背景点
                continue
